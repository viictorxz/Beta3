...
const { magagunmenu } = require('./src/magagumenu')
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Tá carregando ⏳ espera ai que tu né a xuxa não',
				success: 'Deu bom 😎',
                                levelon: '❬ ✔ ❭ *habilitar Level*',
				leveloff: ' ❬ X ❭  *desabilitar Level*',
				levelnoton: '❬ X ❭ *level não ativo*',
				levelnol: '*Pqp kkkkkkk tu é level* 0 ',
				error: {
					stick: '[❗] Vai dar não, deu erro nessaporra 😕',
					Iv: 'Esse link ai não presta não ⛔️'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ⛔️',
					ownerG: '[❗] Não vai dar não, só o dono do grupo pode fazer isso 🙃',
					ownerB: '[❗] Só o Extrupicio pode fazer isso 🙃',
					admin: '[❗] Mano você é comum, esse comando é apenas pra administrador 😕',
					Badmin: '[❗] Primeiro faz o BOT virar ADM garaiii 😡',
                                        daftarB: `── 「REGISTRE-SE」 ──\nÉ o seguinte deixa eu te mandar a real \nTem que se registrar primeiro \n\nComando : ${prefix}Registro nome|idade\nExemplo : ${prefix}Re Magagu|35`,
				}
			}
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				case 'Desprover':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Vacilou, agora tu é só um membro comum 🙁')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Então tu se arrependeu e não quer mais esse ADM? Avisei otário 😼\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`Então tu se arrependeu e não quer mais esse ADM? Avisei otário 😼 @${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                                case 'randomhentai':
                                ...
                                case 'loli':
                                        gatauda = body.slice(6)
                                        if (!isUser) return reply(from, 'Vejo que és um ser de cultura refinada 🧐', id)
                                        reply(mess.wait)
            ...
            break
                  case 'Promover':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Quem tu desejas promover meu nobre? 🧐')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Tu axa que ele merece? Mais tu pediu é uma ordem:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Tu axa que ele merece? Mais tu pediu é uma ordem:@${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
                ...
                break
                   ...
                   break
                   case 'waifu': 
            if (!isGroupMsg) return client.reply(from, 'Esse é famoso 😎', id)
            ...
            break
             case 'milf':  
     if (!isBotGroupAdmins) return client.reply(from, 'Agora você falou a minha língua 😈', id)
            ...
            break
              case 'husbu':
             if (!isBotGroupAdmins) return client.reply(from, 'Pivete tu só dar orgulho 🤩', id)
            ...
            break
				case 'cosplay': 
     if (!isGroupMsg) return client.reply(from, 'Material de qualidade  😊', id) 
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`Deu ruim na hora de converter ${tipe} para o sticker`)
							})
							.on('end', function () {
								console.log('Finish')
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply(`Parâmetro incorreto \nCommand : ${prefix}Registro nome|idade\nContoh : ${prefix}Registro Magagu|35`)
					var reg = body.slice(8)
					var jeneng = reg.split("|")[0];
					var umure = reg.split("|")[1];
						user.push(sender)
						fs.writeFileSync('./database/json/user.json', JSON.stringify(user))
						client.sendMessage(from, `\`\`\`Tá porra menó agora tu faz parte            Codinome:MamaAki\`\`\`\n\n\`\`\`Em: ${date} ${time}\`\`\`\n\`\`\`[Nama]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Idade]: ${umure}\`\`\`\n\`\`\`Liberou os poderes garaiii\`\`\`\n\`\`\`Agora\`\`\`\n\`\`\`envia ${prefix}Ajuda\`\`\`\n\`\`\`\nFDPs Registrados ${user.length}\`\`\``, text, {quoted: mek})
					break
                                case 'Bem Vindo':
					if (!isGroup) return reply(mess.only.group)
                                        if (!isUser) return reply(mess.only.daftarB)
					...
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Agora tá valendo 😎👍')
						welkom.push(from)
				...
				if (!isUser) return reply(mess.only.daftarB)
			       ...
			       reply(anu.result.jadwalTV)
					break
                                case 'afk':
                                        tels = body.slice(4)
                                        if (args.length < 1) return reply('Esse aí quer paz ')
                                        if (!isUser) return reply(mess.only.daftarB)
                                        var nom = mek.participant
                                        const tag = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} *TO AFK BLZ ${tels} NAUM PERTURBE FDPs😡*`,
